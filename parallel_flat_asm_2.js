...
// Tableau de toutes les pièces, finalement passé en global pour avoir formes_places
var lpieces;
...
function initPieces() {
  ...
  // (5) Création, pour chaque forme de chaque pièce, de la liste des places possibles dans le puzzle
  lpieces.forEach(piece => {
    piece.formes_places = [];
    piece.formes1D.forEach(({forme1D, w}) => {
      let forme_places = [];
      
      for(let p =0; p < gl-gp; p++){
          let bCanInsert = true;
          
          if( (p === 0) && (puzzle[0] === 0) && (forme1D[0] !== 0) ) {
            bCanInsert = false;
          } else if ((p % gw) + w > gw) {
            bCanInsert = false;
          } else if (p + forme1D[gp-1] >= gl) {
            bCanInsert = false;
          } else {
            for(let i2=0; (i2 < gp) && bCanInsert; i2++){      
              bCanInsert = (puzzle[p+forme1D[i2]] === 0);
            }
          }
      ...
      piece.formes_places.push(forme_places);
    });
  });
...
function testInsertPuzzle(npiece) {
  if (npiece >= npieces) {
    ...
    for(let iforme=0; iforme<nformes; iforme++) {
      let forme_places = lpieces[npiece].formes_places[iforme];
      let nforme_places = forme_places.length;
      for(let ip = 0; ip < nforme_places; ip++){
        let p = forme_places[ip];
